...
intruc = Image("images\\inero.JPG",game)
intruc.resizeTo(game.width,game.height)
intruc.visible = False
store = Image("images\\store.JPG",game)
store.resizeTo(game.width,game.height)
store.visible = False
#############################################
firework=[]
...
while not game.over:
    ...
    if instructions.collidedWith(mouse) and mouse.LeftButton:
        intruc.visible = True
        intruc.draw()
        store.visible = False
        story.visible = True
        instructions.visible = False
        story.moveTo(1120,850)
        play.moveTo(1300,770)
    if story.collidedWith(mouse) and mouse.LeftButton:
        ...
        story.visible = False
        instructions.visible = True
        play.moveTo(1300,700)
        instructions.moveTo(1120,600)
...
while not game.over:
    ...
    if landed and bunnyr.collidedWith(mushroom) or bunnyl.collidedWith(mushroom):
    ...
    if landed and bunnyl.collidedWith(mushroom2) or bunnyr.collidedWith(mushroom2):
...
while not game.over and heartcount>1:
    ...
    if not landed:#is jumping
        ...
        if bunnyr.collidedWith(mushroom) or bunnyl.collidedWith(mushroom):
            ...
            explosion.visible = True
            mushroom.visible = False
            mushroom.moveTo(game.width+50,710)
            mushroom.visible = True
            carrotcount+=1
            x = randint(1,10)
            mushroom.setSpeed(x,90)
    ...
    if bunnyr.isOffScreen("right") or bunnyl.isOffScreen("right"):
        mushroom.moveTo(game.width+500,710)
    ...
    if mushroom.isOffScreen("left"):
        x = randint(1,10)
        mushroom.setSpeed(x,90)
        mushroom.moveTo(game.width+100,710)
...
#mushrooms
    ...
    if hit:
        ...
        hit = False
    if wolfhealth<1:
        wolf.moveTo(game.width+300,wolf.y)
        explosion.visible = True
        wolfhealth = 3
